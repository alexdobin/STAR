...
When chimeric detection is switched on, STAR will output normal alignments into \ofilen{Aligned.*.sam/bam}, and will output chimeric alignments into a separate file but can also be added to the main BAM file.
To obtain the Chimeric alignment in SAM format, the \opt{chimOutType} needs to be set to \optv{SeparateSAMold}.
