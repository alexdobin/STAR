...
//         ind0a[ii]=-1;//this is needed in case "AAA...AAA",i.e. indPref=0 is not present in the genome for some lengths
    ...
    *P->inOut->logStdOut << timeMonthDayTime(rawTime) <<" ... Generating Suffix Array index\n" <<flush; 
    ...
    uint isareport=0;
    uint nind=P->genomeSAindexNbases;
...
//         };
        ...
        {//skip indices with the same prefix
            ...
            while (isa<P->nSA && funCalcSAiFromSA(G,SA1,isa,nind,P,iL4)==indFullPrev && iL4==iL4prev)
            {
                isa+=isastep;
            };
            if (isa>=P->nSA)
            {
                isa=P->nSA-1;
                indFull=funCalcSAiFromSA(G,SA1,isa,nind,P,iL4);
            } else
            {//binary search
                uint i1=isa-isastep;
                uint i2=isa;
                while (i1+1<i2)
                {
                    ...
                    if ((indFull=funCalcSAiFromSA(G,SA1,isa,nind,P,iL4))==indFullPrev && iL4==iL4prev)
                    {
                        i1=isa;
                    } else
                    {
        ...
        for (uint iL=0; iL < nind; iL++) {//calculate index
//             {//testing: old way
//                 indPref1 <<= 2;
// 
...
//             };
            ...
            uint indPref = indFull >> (2*(nind-1-iL));
//             if (indPref!=indPref1)
...
//                         cout<< iL <<" "<< isa <<" "<< indPref <<" "<<indPref1<<endl;
            ...
            if ( indPref > ind0[iL] || isa==0 ) {//new && good index, record it
                //testing
//                 if (funCalcSAiFromSA(G,SA1,isa,iL+1,P)!=indPref)
//                     cout<< iL <<" "<< isa <<" "<< indPref <<" "<<funCalcSAiFromSA(G,SA1,isa,iL+1,P)<<endl;
