...
void ChimericAlign::chimericJunctionOutput(fstream &outStream)
{
    outStream << P.chrName[al1->Chr] <<"\t"<< chimJ1 - P.chrStart[al1->Chr]+1 <<"\t"<< (al1->Str==0 ? "+":"-") \
        ...
        <<"\t"<< al2->exons[0][EX_G] - P.chrStart[al2->Chr]+1 <<"\t"<< al2->generateCigarP() <<"\n"; //<<"\t"<< al1->exons[0][EX_iFrag]+1 --- no need for that, since al1 is always on the first mate
};
